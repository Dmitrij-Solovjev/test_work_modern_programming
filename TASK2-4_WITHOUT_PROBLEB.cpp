...
struct str{
    ...
    void count_to_dec(vector <book*> &shell, string ss, atomic <int> &ulalala) {
        int i = 0, len = 0;
        bool check = true;
        ulalala =1+ulalala;
        if (WITH_LOG) {
            cout << "                                                                      I am here: "<< ulalala<<endl;
        }
        while (ulalala > 0) {
...
int main() {
    ...
    thread thr_search_book(&str::count_to_dec, &search_book,
                                ref(shell), typeid(book).name(), ref(ulalala));
    thread thr_search_st_book(&str::count_to_dec, &search_st_book,
                                ref(shell), typeid(Student_book).name(), ref(ulalala));
...
//    атомик инт инитед = 1.
//    твои 2 потока делают инитед++ в начале работы.
//    мэйн после старта потоков спит и проверяет что инитед == 3
//    когда ==3 идет дальше
}
